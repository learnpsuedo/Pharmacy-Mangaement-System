...
public Cart deleteCartDrug(@PathVariable int drugId ,@PathVariable int cartId) throws CustomException {
	...
	list.removeIf(d->(d.getDrugId()==drugId));
...
public Cart increaseDrug(@PathVariable int drugId,@PathVariable int cartId) throws CustomException {
	...
	List<Medicines> oldMedicine3 =cart.getMedicines();
		...
		return cartService.updateCart(drugId, cart);
}

//decrease item quantity in cart from cartId and  productId
@PutMapping("/decreaseQuant/{drugId}/{cartId}")
public Cart decreaseDrug(@PathVariable int drugId,@PathVariable int cartId) throws CustomException {
		Cart cart =cartService.getCartById(cartId);
		List<Medicines> oldMedicine3 =cart.getMedicines();
     		for (Medicines m : oldMedicine3) {
